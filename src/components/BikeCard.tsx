...
export default function BikeCard({ bike }: BikeCardProps) {
  ...
  const showDetails = () => {
    toast({ 
      title: `Details for ${bike.name}`, 
      description: 'This feature is coming soon!'
    });
  };

  return (
    <Card className="flex flex-col overflow-hidden shadow-md transition-transform duration-300 hover:-translate-y-1 hover:shadow-lg">
      ...
      <CardFooter className="flex gap-2 p-6 pt-0">
        <Button className="flex-1" onClick={handleInquiry}>
          Inquire Now
        </Button>
        <Button variant="outline" className="flex-1" onClick={showDetails}>
          View Details
        </Button>
      </CardFooter>
    </Card>
